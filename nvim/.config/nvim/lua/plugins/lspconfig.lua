return {
  ...
  config = function()
    ...
    -- NEW: configure servers via vim.lsp.config (Neovim 0.11+)
    ...
    -- mason-lspconfig v2: only install + auto-enable
    require("mason-lspconfig").setup({
      ensure_installed = vim.tbl_keys(servers),
      -- automatic_enable = true, -- this is the default
    })

    -- Manually setup sourcekit-lsp for Swift (still fine to use lspconfig.setup)
